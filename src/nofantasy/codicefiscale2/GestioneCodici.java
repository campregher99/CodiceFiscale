...
public class GestioneCodici {
	...
	private static final char[] SCHELETRO_CODICE= new char[] { 'c', 'c', 'c', 'c', 'c', 'c', 'n', 'n', 'c', 'n', 'n', 'c', 'n', 'n', 'n', 'c'};
	...
	private boolean convalidaCF(String codice) {
		...
		//controllo che nelle varie posizioni ci sia la tipologia di carattere ceh ci aspettiamo
		...
		//controllo che il giorno sia corretto rispetto al mese e che il carattere per definire il mese sia corretto
		switch (codice.charAt(8)) {
		case 'A'://gennaio 31
			if (!((Integer.valueOf(codice.substring(9, 10)) > 0 && Integer.valueOf(codice.substring(9, 10)) < 32) || (Integer.valueOf(codice.substring(9, 10)) > 40 && Integer.valueOf(codice.substring(9, 10)) < 72))) {
				return false;
			}
			break;
		case 'B'://febbraio 28/29
			if (Integer.valueOf(codice.substring(7, 8))%4 == 0) {
				if (!((Integer.valueOf(codice.substring(9, 10)) > 0 && Integer.valueOf(codice.substring(9, 10)) < 30) || (Integer.valueOf(codice.substring(9, 10)) > 40 && Integer.valueOf(codice.substring(9, 10)) < 70))) {
					return false;
				}
			}else {
				if (!((Integer.valueOf(codice.substring(9, 10)) > 0 && Integer.valueOf(codice.substring(9, 10)) < 29) || (Integer.valueOf(codice.substring(9, 10)) > 40 && Integer.valueOf(codice.substring(9, 10)) < 69))) {
					return false;
				}
			}
			break;
		case 'C'://marzo 31
			...
			break;
		case 'D'://aprile 30
			if (!((Integer.valueOf(codice.substring(9, 10)) > 0 && Integer.valueOf(codice.substring(9, 10)) < 31) || (Integer.valueOf(codice.substring(9, 10)) > 40 && Integer.valueOf(codice.substring(9, 10)) < 71))) {
				return false;
			}
			break;
		case 'E'://maggio 31
		...
		case 'T'://dicembre 31
			...
			break;
		default://nel caso in cui la lettera in posizione per il mese non sia corretta ritorna falso
				return false;
		}
